...
class ValoradasProvider {
  ...
  List<Pelicula> _valoradas = new List();

  // Inicializamos la clase
  final _valoradasStreamController =
      StreamController<List<Pelicula>>.broadcast();

  // Este metodo guarda la data cada que se haga una peticion
  Function(List<Pelicula>) get valoradasSink =>
      _valoradasStreamController.sink.add;

  // Este metodo entrega al Widget todo la data que va necesitando
  Stream<List<Pelicula>> get valoradasStream =>
      _valoradasStreamController.stream;

  // Cierra la conexion a los broadcast que ya no se esten utilizando
  void disposedStream() {
    _valoradasStreamController?.close();
  }

  Future<List<Pelicula>> getValoradas() async {
